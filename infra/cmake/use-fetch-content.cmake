...
if(NOT BEMAN_STABLE_ABI_LOCKFILE)
    set(BEMAN_STABLE_ABI_LOCKFILE
        "lockfile.json"
        CACHE FILEPATH
        "Path to the dependency lockfile for beman.stable_abi."
    )
endif()

set(BemanStableAbi_projectDir "${CMAKE_CURRENT_LIST_DIR}/../..")
message(TRACE "BemanStableAbi_projectDir=\"${BemanStableAbi_projectDir}\"")

message(TRACE "BEMAN_STABLE_ABI_LOCKFILE=\"${BEMAN_STABLE_ABI_LOCKFILE}\"")
file(
    REAL_PATH
    "${BEMAN_STABLE_ABI_LOCKFILE}"
    BemanStableAbi_lockfile
    BASE_DIRECTORY "${BemanStableAbi_projectDir}"
    EXPAND_TILDE
)
message(DEBUG "Using lockfile: \"${BemanStableAbi_lockfile}\"")

# Force CMake to reconfigure the project if the lockfile changes
set_property(
    DIRECTORY "${BemanStableAbi_projectDir}"
    APPEND
    PROPERTY CMAKE_CONFIGURE_DEPENDS "${BemanStableAbi_lockfile}"
)
...
function(BemanStableAbi_provideDependency method package_name)
    # Read the lockfile
    file(READ "${BemanStableAbi_lockfile}" BemanExemplar_rootObj)
    ...
    if(BemanExemplar_error)
        message(FATAL_ERROR "${BemanStableAbi_lockfile}: ${BemanExemplar_error}")
    endif()
    ...
    foreach(BemanExemplar_index RANGE "${BemanExemplar_maxIndex}")
        set(BemanExemplar_errorPrefix
            "${BemanStableAbi_lockfile}, dependency ${BemanExemplar_index}"
        )
        ...
        if(method STREQUAL "FIND_PACKAGE")
            if(package_name STREQUAL BemanExemplar_pkgName)
                ...
                string(
                    ...
                    "${BemanExemplar_tag} according to ${BemanStableAbi_lockfile}."
                )
...
cmake_language(
    SET_DEPENDENCY_PROVIDER BemanStableAbi_provideDependency
